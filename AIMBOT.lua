...
-- Fonte Arial + Barra Verde + Lupa + Aba Outros
--==================================================
...
-- Nome "DYV HUB" na barra
local nomeHub = Instance.new("TextLabel")
nomeHub.Size = UDim2.fromOffset(150, 50)
...
searchBtn.Size = UDim2.fromOffset(35, 35)
searchBtn.Position = UDim2.new(1, -50, 0.5, -17.5)
...
-- Campo de pesquisa (escondido por padrão)
local searchBox = Instance.new("TextBox")
searchBox.Size = UDim2.new(1, -20, 0, 40)
searchBox.Position = UDim2.fromOffset(10, 55)
...
content.Position = UDim2.fromOffset(165, 65)
...
contentLine.Position = UDim2.fromOffset(10, 33)
...
contentArea.Position = UDim2.fromOffset(10, 40)
...
local function createSidebarButton(text, y, iconId, callback)
...
--==================================================
-- BOTÃO 3D COM DESCRIÇÃO
--==================================================

local function makeBtn3D(parent, text, descricao, imageId)
	local b = Instance.new("TextButton")
	b.Size = UDim2.new(1, -6, 0, 55)
	...
	shadow.Size = UDim2.new(1, -6, 0, 55)
	...
	-- Descrição (embaixo do texto, fonte pequena)
...
--==================================================
-- VARIÁVEL PRA GUARDAR FUNÇÕES DA ABA ATUAL
-- (pra lupa filtrar)
--==================================================
...
local function buildPlayersList()
	...
	local teleportBtn, teleportLbl = makeBtn3D(contentArea, "TELEPORTAR", "Ir até o player selecionado", ICONE)
	teleportBtn.Name = "TeleportBtn"
	teleportBtn.Position = UDim2.new(0, 0, 1, -62)
	teleportBtn.Size = UDim2.new(1, 0, 0, 55)
...
local function buildPlayer()
	...
	listJ.Padding = UDim.new(0, 6)
	listJ.SortOrder = Enum.SortOrder.LayoutOrder
	listJ.Parent = scrollJ

	local lowGravBtn, lowGravLbl = makeBtn3D(scrollJ, "GRAVIDADE BAIXA", "Deixa o personagem mais leve", ICONE)
	local noFallBtn, noFallLbl   = makeBtn3D(scrollJ, "SEM DANO DE QUEDA", "Impede de morrer ao cair", ICONE)
...
local function buildFunctions()
	...
	listF.Padding = UDim.new(0, 6)
	listF.SortOrder = Enum.SortOrder.LayoutOrder
	listF.Parent = scrollF

	local fpsBtn, fpsLbl         = makeBtn3D(scrollF, "FPS / RELÓGIO", "Mostra FPS e horário", ICONE)
	local invisBtn, invisLbl     = makeBtn3D(scrollF, "TRANSPARENTE", "Deixa você invisível", ICONE)
	local speedBtn, speedLbl     = makeBtn3D(scrollF, "SPEED", "Aumenta a velocidade", ICONE)
	local viewBtn, viewLbl       = makeBtn3D(scrollF, "VER PLAYER", "Olha pelos olhos do player", ICONE)
	local calcBtn, calcLbl       = makeBtn3D(scrollF, "CALCULADORA", "Calcula contas", ICONE)
	local jumpBtn, jumpLbl       = makeBtn3D(scrollF, "PULO INFINITO", "Pula sem parar no ar", ICONE)
	local flyBtn, flyLbl         = makeBtn3D(scrollF, "FLY MOBILE", "Voar com botões", ICONE)
	local musicBtn, musicLbl     = makeBtn3D(scrollF, "TOCAR MÚSICA", "Toca música perto de você", ICONE)
	local followBtn, followLbl   = makeBtn3D(scrollF, "SEGUIR PLAYER", "Segue o player escolhido", ICONE)
	local rgbBtn, rgbLbl         = makeBtn3D(scrollF, "RGB", "Cor do boneco muda", ICONE)
	local noclipBtn, noclipLbl   = makeBtn3D(scrollF, "NOCLIP (PAREDES)", "Atravessa paredes", ICONE)

	registrarBotao("FPS / RELÓGIO", "Mostra FPS e horário", fpsBtn)
	registrarBotao("TRANSPARENTE", "Deixa você invisível", invisBtn)
	...
	local function applyInvis()
		...
		for _, part in ipairs(char:GetDescendants()) do
			if part:IsA("BasePart") then
				if invisOn then part.Transparency = 0.7
				else if part.Name ~= "HumanoidRootPart" then part.Transparency = 0 end end
			end
			if part:IsA("Decal") then part.Transparency = invisOn and 1 or 0 end
		end
	end
	...
	invisBtn.MouseButton1Click:Connect(function()
		invisOn = not invisOn
		applyInvis()
		invisLbl.Text = invisOn and "TRANSPARENTE: ON" or "TRANSPARENTE"
	end)
...
local function buildPreferences()
	...
	-- FPS + 40
	local fps40Btn, fps40Lbl = makeBtn3D(scrollPre, "FPS + 40", "Melhora o desempenho", ICONE)
	...
	-- TELA ESTICADA
	local fovBtn, fovLbl = makeBtn3D(scrollPre, "TELA ESTICADA (FOV)", "Aumenta a visão", ICONE)
	local fovOn = false
	...
	-- CÉU ENTARDECER
	local sunsetBtn, sunsetLbl = makeBtn3D(scrollPre, "CÉU ENTARDECER", "Ativa o pôr do sol", ICONE)
...
local function buildOutros()
	...
	local spawnBtn, spawnLbl   = makeBtn3D(scrollO, "TELEPORTAR PRA SPAWN", "Volta pro início do mapa", ICONE)
	local cameraBtn, cameraLbl = makeBtn3D(scrollO, "MODO CÂMERA", "Câmera livre pelo mapa", ICONE)
	local otimBtn, otimLbl     = makeBtn3D(scrollO, "OTIMIZAR GRÁFICOS", "Deixa o jogo mais leve", ICONE)
	local copiarBtn, copiarLbl = makeBtn3D(scrollO, "COPIAR INFO DO JOGO", "Copia dados do servidor", ICONE)
...
<font size="11" color="rgb(180,180,180)">Ajuda na comunidade e suporte.</font>
...
--==================================================
-- BOTÕES DO SIDEBAR (6 abas)
--==================================================

createSidebarButton("Players", 10, ICONE, function()
	selectTab(1, "PLAYERS")
	buildPlayersList()
end)

createSidebarButton("Jogador", 50, ICONE, function()
	selectTab(2, "JOGADOR")
	buildPlayer()
end)

createSidebarButton("Funções", 90, ICONE, function()
	selectTab(3, "FUNÇÕES")
	buildFunctions()
end)

createSidebarButton("Preferências", 130, ICONE, function()
	selectTab(4, "PREFERÊNCIAS")
	buildPreferences()
end)

createSidebarButton("Outros...", 170, ICONE, function()
	selectTab(5, "OUTROS")
	buildOutros()
end)

createSidebarButton("Créditos", 210, ICONE, function()
	selectTab(6, "CRÉDITOS")
	buildCredits()
end)

--==================================================
-- LÓGICA DA LUPA (FILTRA ABA ATUAL)
--==================================================
